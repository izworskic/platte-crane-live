...
export interface CraneSurvey { date:string; datePrecision:'exact'|'survey-week-bin'; year:number; surveyWeek:number; count:number; lowerBound:number|null; upperBound:number|null; error:number|null; surveyType:string; sourceUrl:string; sourceOrganization:string; retrievedAt:string; notes:string; }
