...
export default function CameraCard(){
  const [expectation,setExpectation]=useState<Expectation>({label:'Checking camera timing',text:'Using Central Time to set expectations for what the river camera should show.',className:''});
  useEffect(()=>setExpectation(cameraExpectation()),[]);
  return <section className="panel camera-card">
    ...
    <div className={`camera-expectation ${expectation.className}`} aria-live="polite"><span>WHAT SHOULD I EXPECT NOW?</span><strong>{expectation.label}</strong><p>{expectation.text}</p></div>
